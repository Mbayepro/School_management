...
const init = async () => {
  ...
  try {
      ...
      if (filterClasse) filterClasse.addEventListener('change', renderElevesList);
      if (filterNiveau) filterNiveau.addEventListener('change', () => { populateClasseFilter(); renderElevesList(); });
      if (searchEleve) searchEleve.addEventListener('input', renderElevesList);
        ...
        eleveForm.addEventListener('submit', async (e) => {
          ...
          try {
            if (currentId) {
              ...
              // Update local cache
              const updated = data;
              // Remove from old class list if needed (complex, so just reload or simplify)
              // For simplicity, we'll reload data to be safe, or update optimistically
              // Optimistic update:
            ...
            console.error(err);
            if (formMessage) { formMessage.textContent = "Erreur: " + err.message; formMessage.style.display = "block"; }
          }
        });
      }

      // --- ID CARD MODAL EVENTS ---
      if (closeIdCardModal) {
          closeIdCardModal.addEventListener('click', () => { 
              console.log('Close modal clicked (X)');
      ...
      if (closeIdCardBtn) {
          closeIdCardBtn.addEventListener('click', () => { 
              console.log('Close modal clicked (Btn)');
              if(idCardModal) idCardModal.classList.add('hidden'); 
              selectedEleve = null; 
          });
      }

      if (idCardModal) window.addEventListener('click', (e) => { 
      ...
      // --- PRINT SINGLE ---
      if (printIdCardBtn) {
          printIdCardBtn.addEventListener('click', async () => {
              console.log('Click print single', selectedEleve);
              if (!selectedEleve) {
              ...
              if (!printableContainer) {
                  alert("Erreur critique: Conteneur d'impression introuvable.");
                  return;
              }
              
              printableContainer.innerHTML = '';
              
              const card = generateCardHtml(selectedEleve);
              // Centrer pour impression unique
      ...
      if (selectAllBtn) {
        selectAllBtn.addEventListener('click', () => {
          const checkboxes = document.getElementById('elevesList').querySelectorAll('input[type="checkbox"]');
      ...
      if (printSelectedBtn) {
          printSelectedBtn.addEventListener('click', async () => {
            const checkboxes = document.getElementById('elevesList').querySelectorAll('input[type="checkbox"]:checked');
            if (checkboxes.length === 0) return alert("Veuillez sélectionner au moins un élève.");
            
            const printableContainer = document.getElementById('printableCardsContainer');
            if (!printableContainer) return alert("Erreur: Container d'impression absent.");

            printableContainer.innerHTML = '';
            
            for (const cb of checkboxes) {
                 const eleveData = JSON.parse(cb.dataset.eleve);
                 const card = generateCardHtml(eleveData);
                 printableContainer.appendChild(card);
            }
      ...
      if (!ecoleId && errEl) {
        errEl.textContent = "Compte non associé à une école.";
      ...
      try { await loadData(); } catch (err) { console.error(err); if (errEl) { errEl.textContent = "Erreur chargement données."; errEl.style.display = "block"; } }
      ...
      async function loadMetrics() {
        ...
        if (classIdsAll.length === 0) return;
      ...
      console.error("FATAL ERROR IN ELEVES.JS:", globalErr);
      alert("Erreur critique dans eleves.js: " + globalErr.message);
  }
};
